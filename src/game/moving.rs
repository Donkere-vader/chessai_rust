    ...
    pub fn do_move(&mut self, mve: &Move) {
        ...
        match mve_type {
            MoveType::Standard => {
                ...
                if let Some(p) = self.board[mve.to[1]][mve.to[0]] {
            ...
            MoveType::EnPassant => {
                // update score
                if let Some(p) = self.board[mve.from[1]][mve.to[0]] {
                    take_piece = Some(p);
